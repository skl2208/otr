...
if (isset($_POST["inSearch"])) {
    ...
    if (!empty($search_text)) {
        $search_condition = " AND (link_name LIKE '%" . $search_text . "%' OR link_url LIKE '%" . $search_text . "%')";
    }
}
...
?>
<h3>LINK เวปไซต์อื่น</h3>
<div><button class="btn1" onclick="javascript:addLinkBanner();">เพิ่มรายการ</button></div>
<div>
    <form method="POST" action="list_link.php">
...
<?php
$sql = "SELECT seq,link_name,link_url,id,status FROM link " . $condition . " ORDER BY seq";
...
?>
        <div class="d-table w-100 mt-1">
            <div style="display:table-row">
                ...
                <div style="width:35%;" class="ps-2 smallerFont head-row">ชื่อลิงค์</div>
                <div style="width:35%;" class="ps-2 smallerFont head-row">URL ที่จะเรียกไป</div>
    ...
    <?php
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<div style=\"width:10%;\" class=\"ps-2 border-bottom dp-data\"><button class=\"btnEDIT\" onclick=\"javascript:editLinkBanner('" . $row["id"] . "');\">" . $i++ . "</button></div>";
            echo "<div style=\"width:10%;\" class=\"ps-2 border-bottom dp-data\">" . $row["seq"] . "</div>";
            echo "<div style=\"width:35%;\" class=\"ps-2 border-bottom dp-data\">" . $row["link_name"] . "</div>";
            echo "<div style=\"width:35%;\" class=\"ps-2 border-bottom dp-data\">" . $row["link_url"] . "</div>";
