...
<script>
    ...
    function validate_data() {
        ...
        const webName = document.getElementById("webpage_name").value;

        if (webName == "" || webName == null) {
            showAlert.show("กรุณาใส่ชื่อหน้าเวป", "DANGER");
            document.getElementById("webpage").focus();
        } else {
            ...
            manage_webpage.save(document.forms['formedit']);
        }
    }
    ...
    function goPreviewHTML() {
        ...
        manage_webpage.previewHTML(document.getElementById("content").value);
    }

    $(function() {
        const id = "<?php if($command=="INSERT") echo ""; else echo $id; ?>";
    ...
    <form method="POST" name="formedit" id="formedit">
        <div class="d-table w-100 mt-3">
            ...
            <div style="display:table-row">
                ...
                <div class="ps-2 smallerFont w-100 column-input"><input type="checkbox" title="ถ้าไม่ระบุจะให้ค่าเริ่มต้นเป็นแสดง" class="ms-1 me-1" id="status1" name="status1" checked onclick="updateCheckBox(this);">แสดง</div>
            </div>
        </div>
        <div class="d-table w-100 mt-3">
        ...
        <input type="hidden" id="id" name="id" value="<?php if($command=="INSERT") {echo "";} else echo $id; ?>">
    </form>
</section>
