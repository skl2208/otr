...
if (isset($_GET["position"])) {
    ...
    if ($position == "A" || $position=="") {
        ...
        $condition1 = " AND position='$position'";
    }
}
...
$sql = "SELECT count(id) AS totalrec FROM residency WHERE 1 " . $condition;
...
<div>
    <button class="btn1" onclick="javascript:window.location.href='addeditresident.php'">เพิ่มรายการ</button>
    <button class="btn1" onclick="javascript:showAlert.show('กำลังพัฒนาระบบการเลื่อนชั้นปีอัตโนมัติ','WARN');">เลื่อนชั้นปีอัตโนมัติ</button>
</div>
<div>
    <form method="GET" name="form0" id="form1" action="list_resident.php">
        ...
        <select id="position" name="position">
            ...
            <?php
            $sql = "SELECT * FROM resident_type ORDER BY position";
            $result0 = $conn->query($sql);
            if ($result0 && $result0->num_rows > 0) {
                while ($row0 = $result0->fetch_assoc()) {
                    echo "<option value=\"" . $row0["position"] . "\"" . ($position == $row0["position"] ? "selected" : "") . ">" . $row0["position"] . "</option>";
                }
            }
            ?>
...
<?php
$sql = "SELECT * FROM residency WHERE 1 " . $condition . " ORDER BY position DESC,name,surname LIMIT " . (($pageno - 1) * $numberRowPerPage) . "," . $numberRowPerPage;
$result = $conn->query($sql);
...
?>
        <div class="d-table w-100 mt-1">
            <div style="display:table-row">
                ...
                <div style="width:40%;" class="ps-2 smallerFont head-row">กลุ่ม</div>
    ...
    <?php
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<div style=\"width:10%;\" class=\"ps-2 border-bottom dp-data\"><button class=\"btnEDIT\" onclick=\"javascript:window.location.href='addeditresident.php?id=". $row["id"] . "'\">" . $i++ . "</button></div>";
            echo "<div style=\"width:40%;\" class=\"ps-2 border-bottom dp-data\">" . $row["titlename"] . $row["name"] . " " . $row["surname"] . "</div>";
            echo "<div style=\"width:40%;\" class=\"ps-2 border-bottom dp-data\">" . $row["position"] . "</div>";
