    ...
    <div class="container-fluid">
        <div class="row mx-lg-5 mx-md-1 mx-sm-0 template-2 pb-3 shadow-sm">
            <div class="col-12 mt-lg-3 mt-md-2">
                <h2 class="topic-header">งานวิจัยระดับประเทศ</h2>
            </div>
            <?php
            $sql = "SELECT count(id) as Totalrec FROM research WHERE status='Y' AND group_name='งานวิจัยระดับประเทศ'";
            ...
            if($result0 && $result0->num_rows > 0) {
                ...
                $sql = "SELECT * FROM research WHERE status='Y' AND group_name='งานวิจัยระดับประเทศ' ORDER BY create_date DESC,topic ";
