    ...
    <div class="container-fluid">
        <div class="row mx-lg-5 mx-md-1 mx-sm-0 template-2 pb-3 shadow-sm">
            <div class="col-12 mt-lg-3 mt-md-2">
                <h2 class="topic-header">รายชื่อแพทย์ประจำบ้านต่อยอด</h2>
            </div>
            <?php
            $sql = "SELECT * FROM resident_group WHERE group_name LIKE 'แพทย์ประจำบ้านต่อยอด%' AND status='Y' ORDER BY group_name DESC ";
            $result = $conn->query($sql);
            if ($result && $result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    echo "<div class=\"col-lg-9 col-md-9 col-12 mt-2 mb-2 text-center\">";
                    echo "<h5>".$row["group_name"]."</h5></div>";
                    echo "<div class=\"col-lg-3 col-md-3 col-12 mt-2 mb-2 text-center\">";
